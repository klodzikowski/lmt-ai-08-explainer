# MNIST Network Explainer

An interactive worked example for the Class 8 homework ("vibe-code a tool that helps you
understand our neural network"). Draw a digit and watch it travel through the course
network — the 784-64-10 dense net — from raw pixels to hidden-layer activations to the
final 10 probabilities. Everything runs live in the browser from one self-contained
`index.html`: real trained weights, real forward passes, no internet needed.

## How to use it in class

- Open `index.html` (double-click, or serve via GitHub Pages — it works offline either way).
...
- Teaching moments that come up naturally: clear the canvas and note the network *still*
  ...
  shape and watch the probabilities split; hover over hidden-layer squares to see
  individual neurons fire or stay silent.
...
passes the total through ReLU (negative totals become 0 — the neuron stays silent). The
